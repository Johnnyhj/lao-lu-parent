    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            if (p.valueFieldID)
            {
                ...
                /**
				 * 
				 * 为了实现同一个form中多个combobox的隐藏域拥有不同ID，同一个NAME，在属性中添加valueFieldName，
				 * 如果需要多个combobox的隐藏域拥有不同ID，同一个NAME，在创建cmbobox时，必须定义valueFieldID
				 * 和valueFieldName属性，如果需要ID和NAME相同，只需要定义valueFieldID，不要定义valueFieldName
				 * 
				 * edit by tongfang
				 * 
				 * 2012.09.14
				...
				if(p.valueFieldName){
					g.valueField[0].name = p.valueFieldName;
				}else{
					g.valueField[0].name = p.valueFieldID;
				}
                /** ************************ END *********************** */
            }
            else
        ...
        _setValue: function (value)
        {
            ...
            else
            {
                ...
                var targetdata = value.toString().split(p.split);
               /**
				 * modify by:yanglin 修改前 setValue传入进来的值，首先tr中<input
				 * type="checkbox"/>中全部不选中，判断传入的value值与tr的value值相等，设置checkbox为checked
				 * 
				 * 
				 * $("table.l-table-checkbox :checkbox",
				 * g.selectBox).each(function () { this.checked = false; }); for
				 * (var i = 0; i < targetdata.length; i++){
				 * $("table.l-table-checkbox tr[value=" + targetdata[i] + "]
				 * :checkbox", g.selectBox).each(function () { this.checked =
				 * true; }); }
				 * 
				 */
                /**
				 * 修改后 修改tr中a标签中图片的位置来显示checked
				 */                 
                $("table.l-table-checkbox a", g.selectBox).each(function () { $(this).addClass("l-checkbox"); });
                for (var i = 0; i < targetdata.length; i++){
                       $("table.l-table-checkbox tr[value=" + targetdata[i] + "] a", g.selectBox).each(function () { 
        ...
        setTree: function (tree)
        {
            ...
            /********************************************************/
            /**
             * 如果用户自己实现了onAfterAppend则调用用户实现的方法
             * 
             * 修改前的代码
             * 
             * edited by igood 2014-05-22
             * 
             */
...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
        ...
        fixedType: null,            // 在固定的位置显示, 可以设置的值有n, e, s, w, ne, se, sw,
									// nw
    ...
    {	
    	//修改open框底部按钮不中间对齐和位置反序的问题
    	if(p.buttons){
    		var reverseButtons = p.buttons.reverse();
    		p.buttons = reverseButtons;
    	}
...
(function ($) {
    ...
    $.ligerui.controls.Filter.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _setFields: function (fields) {
            var g = this, p = this.options;
            if (g.group)
                g.group.html(g._bulidGroupTableHtml());
            else
        ...
        addGroup: function (group) {
            ...
            var altering = !group.hasClass("l-filter-group-alt");
            groupHtmlArr.push(g._bulidGroupTableHtml(altering, true));
            groupHtmlArr.push('</td></tr>');
            lastrow.before(groupHtmlArr.join(''));
        },
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        onBeforeCheckRow: null,                 // 选择前事件，可以通过return
												// false阻止操作(复选框)
        onCheckRow: null,                    // 选择事件(复选框)
        onBeforeCheckAllRow: null,              // 选择前事件，可以通过return
												// false阻止操作(复选框 全选/全不选)
        onCheckAllRow: null,                    // 选择事件(复选框 全选/全不选)
        onBeforeShowData: null,                  // 显示数据前事件，可以通过reutrn
													// false阻止操作
        ...
        where: null,                           // 数据过滤查询函数,(参数一 data item，参数二
												// data item index)
    ...
    $.ligerDefaults.GridString = {
        ...
        pageStatMessage: '显示从{from}到{to}，总 {total} 条 。每页',
        /****************************end***************************/
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadServerData: function (param, clause)
        {
            ...
            var ajaxOptions = {
                ...
                complete: function (xhr, textStatus)
                {
    				 ...
    				 * now: editor: tongfang
    				 * 
    				 * 当使用ajax提交请求时，如果session过期，跳转到登录页面
    				 * 
    				 * 2012-11-06
    				 */
                	/*if(xhr && xhr.responseText){
                		var resJson = $.parseJSON(xhr.responseText);
                		if(resJson && resJson.IS_REQUEST_TIMEOUT){
                			window.location.href = "userController.do?method=index";
                		}
                	}*/
                	/** ************************ END *********************** */
        ...
        changePage: function (ctype)
        {
            ...
            /*
             * yanglin
             * 当前页和新操作的页相同，则不进行查询，此时针对点击查询按钮进行查询时，会导致有查询条件而不进行操作
        ...
        _initFootbar: function ()
        {
            ...
            if (p.usePager)
            {
                 ...
                 * 2013-01-07
                 * **/
                $('.l-bar-selectpagesize', g.toolbar).append("条");
                /****************************end***************************/
        ...
        _onClick: function (e)
        {
            ...
            else if (src.hcelltext) // 排序
            {
                 ...
                 */
                 ...
                 /*
                  * yanglin
                  * 修改后，后台排序列的name为传入的参数sortName的值
                  * now
                  */
                 var columnName = src.column.sortName;
                 if (columnName == null || columnName == ""){
                	 columnName = src.column.name;
                 }
...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        moveToPrevTabItem: function ()
        {
            ...
            var leftList = []; // 记录每个tab的left,由左到右
        ...
        moveToNextTabItem: function ()
        {
            ...
            var mainwidth = g.tab.width();
            var leftList = []; // 记录每个tab的left,由右到左
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
				 ...
				 * edited by tongfang 2013-04-01 恢复成原来的代码
				 */
                ...
                /**
				 * now: editor: tongfang
				 * 为了实现树节点可以取嵌套对象中的属性，例如：修改前只能取：data.itemName,
				 * 修改之后可以取：data.itemInfo.itemName, 目前只能多取一层
				 * 
				 */
                
                /*
				 * if(p.textFieldName.indexOf(".") > 0){ var parent =
				 * p.textFieldName.split(".")[0]; var child =
				 * p.textFieldName.split(".")[1]; treehtmlarr.push('<span>' +
				 * o[parent][child] + '</span></div>'); }else{
				 * treehtmlarr.push('<span>' + o[p.textFieldName] + '</span></div>'); }
				 */
                /**
				 * now editor: yanglin 新增属性节点的提示 onmouseover 鼠标移在当前span上提示显示
				 * onmouseout 鼠标移开时提示消失
				 * 
				 * removed by tongfang 2013-04-01 恢复成原来的代码
				 */
                /*treehtmlarr.push("<span ");
                // treehtmlarr.push('class="message"');
                treehtmlarr.push('style="position:absolute;margin-left:45px;"');
                treehtmlarr.push(' onmouseover="' + p.onmouseover + ';"');
                treehtmlarr.push(' onmouseout="' + p.onmouseout + ';" ');
                treehtmlarr.push('id=' + o[p.idFieldName] + ' ');
                treehtmlarr.push(">");
                if(p.textFieldName.indexOf(".") > 0){
                	var parent = p.textFieldName.split(".")[0];
                	var child = p.textFieldName.split(".")[1];
                	treehtmlarr.push(o[parent][child] + '</span></div>');
                }else{
                	treehtmlarr.push(o[p.textFieldName] + '</span></div>');
                }*/
                ...
                if ($(obj).hasClass("l-checkbox"))
                {
                    if (p.autoCheckboxEven)
                    {
                        ...
                        if ($(obj).hasClass("l-checkbox-unchecked"))
                        {
                            ...
                            /***
                             * 
                             * 树本身支持自定义onCheck事件，但是下拉树不支持，这里添加对下拉树的onCheck事件的
                             * 支持，需要在tree属性下添加onCheckExtend方法，具体方式可参考taskAdd.js中区域下拉树
                             * 
                             * added by igood 2014-05-22
                             * 
                             */
                            if(p && p.onCheckExtend){
                            	p.onCheckExtend(g, { data: treenodedata, target: treeitem[0] }, true);
                            }
                            /********************** end *********************/
                        }
                        // 状态：选中
                        else if ($(obj).hasClass("l-checkbox-checked"))
                        {
                             ...
                             */
                            if(p && p.onCheckExtend){
                            	p.onCheckExtend(g, { data: treenodedata, target: treeitem[0] }, false);
                            }
                            /********************** end *********************/
                        }
                        // 状态：未完全选中
