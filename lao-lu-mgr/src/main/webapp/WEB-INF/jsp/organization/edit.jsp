    ...
    <div class="container-fluid">
        ...
        <c:choose>
            <c:when test="${empty object}">
                <c:set var="action" value="${ctx}/organization/appendChild" />
            </c:when>
            <c:otherwise>
                <c:set var="action" value="${ctx}/organization/update" />
            </c:otherwise>
        </c:choose>
        

        <form id="frm_object" action="${action}" method="post">

            <input type="hidden" name="id" value="${object.id}" />
            <input type="hidden" name="available" value="${object.available}" />
            <input type="hidden" name="parentId" value="${empty parent ? 0 : parent.id}" />
            <input type="hidden" name="parentIds" value="${parent.parentIds}" />
            ...
            <div class="row">
                ...
                <c:if test="${not empty parent}">
                    <div class="col-md-12">
                        <h5>父节点名称： ${parent.name}</h5>
                    </div>
                </c:if>

                <div class="col-md-12">
                    <h5><c:if test="${not empty parent}">子</c:if>名称：</h5>
                    <div class="con-area">
                ...
                <div class="col-md-12">
                    <h5>角色列表：</h5>
                    <input id="btn_add_role" type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target=".bs-example-modal-lg" value="新增角色" style="margin-bottom: 5px;" />
                    ...
                    <div class="con-area">
                        ...
                        <table class="table table-striped b-t b-light text-base">
                            ...
                            <tbody id="tbd_role">
                            <c:if test="${not empty object.sysOrgaRoleRels}">
                                <c:forEach var="rel" items="${object.sysOrgaRoleRels}">
                                    <tr>
                                        <input type="hidden" name="roleId" value="${rel.roleId}">
                            ...
                            <%--<div class="form-group col-md-4 ">
                                <form:select path="roleIds" class="form-control" items="${roleList}" itemLabel="description" itemValue="id" multiple="true" />
                                (按住shift键多选)
                            </div>--%>
                        <div class="clearfix"></div>
                    </div>
            ...
            <div style="padding: 10px; text-align:center">
                <input id="btn_form_submit" type="button" class="btn btn-sub" value="${op}"/>
            </div>
        </form>
    </div>
</div>
...
<script>
    var getTemplateText = function (object) {
        var text = '<tr>' +
                '<input type="hidden" name="roleId" value="{sysRoleId}">' +
            ...
            getUserRoleRel: function () {
                ...
                $.each($trs, function (index, tr) {
                    ...
                    $.each($hidden, function (j, hid) {
                        var $hid = $(hid);
                        object.push('<input type="hidden" name="sysOrgaRoleRels[' + index + '].' + $hid.attr('name') + '" value="' + $hid.val() + '"/>')
                    });
