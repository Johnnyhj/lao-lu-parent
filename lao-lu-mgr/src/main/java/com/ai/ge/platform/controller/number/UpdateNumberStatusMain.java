package com.ai.ge.platform.controller.number;

import com.ai.ge.bus.core.AbilityConstant;
import com.ai.ge.bus.core.AbilityRegister;
import com.ai.ge.bus.core.IAbility;
import com.ai.ge.bus.core.Message;
import com.ai.ge.platform.controller.sys.ServiceParamMain;
import com.ai.ge.platform.model.product.NumberInfo;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Service;

/**
 * Created by guochunhao on 17/5/29.
 */
@Service
@AbilityRegister(name = "updateNumberStatus")
public class UpdateNumberStatusMain extends ServiceParamMain implements IAbility
{
    private Logger logger = LoggerFactory.getLogger(UpdateNumberStatusMain.class);

    @Override
    public Message execute(Message message)
    {
        int resultCode = AbilityConstant.R_SUCCESS;
        String resultMsg = "更新成功";

        try
        {
            NumberInfo number = message.getParam(NumberInfo.class);

            phoneNumService.updateNumber(number, false);

        } catch (Exception e)
        {
            e.printStackTrace();
            logger.error(e.getMessage());

            resultCode = AbilityConstant.R_ERROR_SYSTEM;
            resultMsg = "更新失败";

        } finally
        {
            message.setCode(resultCode);
            message.setMsg(resultMsg);

        }
        return message;
    }
}
